...
func TestNewClientProvider(t *testing.T) {
	baseURL, _ := url.Parse("http://localhost:8080")
	provider := NewClientProvider(*baseURL, confighttp.NewDefaultHTTPClientSettings(), componenttest.NewNopHost(), componenttest.NewNopTelemetrySettings())
...
func TestDefaultClient(t *testing.T) {
	...
	client, err := defaultClient(*endpoint, confighttp.NewDefaultHTTPClientSettings(), componenttest.NewNopHost(), componenttest.NewNopTelemetrySettings())
	require.NoError(t, err)
